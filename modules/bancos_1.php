...
// Módulo para ver, agregar, editar y eliminar bancos de la empresa actual (campos del formulario en líneas separadas y estilo más simple)
...
if (isset($_POST['accion']) && $_POST['accion'] === 'agregar') {
    ...
    $numero_cuenta = preg_replace('/[^0-9]/', '', $_POST['numero_cuenta']); // Solo números
    $titular = trim($_POST['titular']);
    $saldo = floatval(str_replace(',', '.', $_POST['saldo']));

    // Validación para que tipo_cuenta solo acepte "ahorros" o "corriente"
...
function selected_tipo_cuenta($valor, $banco_editar) {
    // Si estamos editando, selecciona lo que tenga el banco
    if (isset($banco_editar['tipo_cuenta'])) {
        return $banco_editar['tipo_cuenta'] === $valor ? 'selected' : '';
    }
    // Si NO estamos editando, por defecto "corriente"
    if ($valor === 'corriente') {
        return 'selected';
    }
    return '';
...
<body>
    ...
    <main class="seccion-clientes">
        <section class="clientes-form">
            ...
            <?php endif; ?>
            <form method="POST" class="formulario formulario-banco" autocomplete="off">
            ...
            <table class="tabla-bancos">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="td-saldo" style="text-align:right;"><?php echo number_format($b['saldo'],2,',','.'); ?></td>
                        <td>
