...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nueva_tasa'])) {
    ...
    if ($stmt->execute()) {
        // Guardar mensaje en sesión (flash) y redirigir para evitar reenvío del formulario
        $_SESSION['mensaje_tasa'] = "Tasa del día actualizada.";
    } else {
        $_SESSION['mensaje_tasa'] = "Error al actualizar tasa: " . $conn->error;
    }
    $stmt->close();

    // Redirigir al mismo script para limpiar POST (PRG pattern)
...
$mensaje_tasa = '';
if (isset($_SESSION['mensaje_tasa'])) {
    $mensaje_tasa = $_SESSION['mensaje_tasa'];
    unset($_SESSION['mensaje_tasa']);
}
...
// Preparar el valor para el input (usar punto decimal para input[type=number])
$input_tasa_value = ($tasa_actual !== '' && $tasa_actual !== null) ? str_replace(',', '.', (string)$tasa_actual) : '';
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    ...
    <main class="dashboard-main">
        ...
        <section class="dashboard-modulos" aria-label="Módulos rápidos">
            ...
            <a href="flujocaja.php" class="modulo-btn" title="Flujo de Caja">
                <span class="modulo-icon">&#128179;&#65039;</span>
                <span class="modulo-text">Flujo Caja</span>
            </a>
